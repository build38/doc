...
Functions marked as `alwaysinline` are skipped as `indirectbr` instructions may prevent inlining
from occur. There are some known limitations on iOS applications built in Release mode. The pass
currently attempts to translate LLVM critical edges too, although this comes at the cost of
possibly breaking some invariances to code motion optimizations that run next. As mentioned above,
